import ..Interpolators: PfullInterpolator, era5_pressure_levels
...
Interpolate exactly to the points of the space whose vertical direction consists
of `PPoint`s.
...
Sample points defined on the vertical direction of a space that contains
`PPoint`s.

The keyword arguments passed to `RealPressureLevelsMethod` is passed to the
constructor of `PfullInterpolator`.
...
function RealPressureLevelsMethod(
    ...
    pfull_intp = PfullInterpolator(compute_pfull!, Y, p, t; pfull_levels)
...
"""
    pressure_space(pfull_intp::PfullInterpolator)
...
Add dimensions relevant to the `space` to the given `nc` NetCDF file. The horizontal
dimensions are `hpts` and the vertical dimension is `vpts`. It is the responsibility of the
user to ensure the `hpts` and `vpts` make sense for the given diagnostic variable.
...
Sampling points along the vertical dimension is specified by `z_sampling_method` (see
`AbstractZSamplingMethod`). The `interpolated_physical_z` keyword argument is an array
containing the physical altitude at each target interpolation point, with dimensions
matching the grid defined by `hpts` and `vpts`.
"""
    ...
    if z_sampling_method isa RealPressureLevelsMethod
        ...
        if !pfull_dimension_exists
            add_dimension!(
                ...
                stored_direction = "increasing",
                standard_name = "air_pressure",
                z_sampling_method.pfull_attribs...,
            )
        end
        return [name]
    end
