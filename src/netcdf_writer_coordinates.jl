...
# Multiple independent columns (multiple points extruded with a vertical grid)
function add_space_coordinates_maybe!(
    nc::NCDatasets.NCDataset,
...
For `MultiPointSpace`, return `nothing`: the columns are fixed, so the `Remapper` takes no
horizontal target coordinates.
"""
