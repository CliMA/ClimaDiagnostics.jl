...
function DiagnosticsHandler(scheduled_diagnostics, Y, p, t; dt = nothing)
    ...
    # broadcasted_expressions contains the un-evaluated expressions, which are then
    # materialized and moved to storage
    ...
    for (i, diag) in enumerate(unique_scheduled_diagnostics)
        ...
        if has_inplace_compute
            ...
            out_or_broadcasted_expr = variable.compute(Y, p, t)
            if out_or_broadcasted_expr isa Base.Broadcast.Broadcasted
                # Case 2b
                broadcasted_expressions[diag] = out_or_broadcasted_expr
                out_field = Base.Broadcast.materialize(out_or_broadcasted_expr)
            else
                # Case 2a
                out_field = out_or_broadcasted_expr
            end
        end
...
function orchestrate_diagnostics(
    ...
    for diag_index in scheduled_diagnostics_keys
        ...
        #
        # 2a. The variable has a `compute` function that returns a Field. In this case, we
        # copy it over to the storage.
        #
        # 2b. The variable has a `compute` function that returns
        # `Base.Broadcast.Broadcasted`. In this case, we update the value in our dictionary
        # and materialize the expression in the next loop.
