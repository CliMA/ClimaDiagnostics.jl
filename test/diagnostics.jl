...
@testset "Diagnostics" begin
    point_space = ClimaCore.Spaces.PointSpace(
        ClimaComms.context(),
        ClimaCore.Geometry.ZPoint(1.0),
    )
    column_space = ColumnCenterFiniteDifferenceSpace()

    t0 = 0.0
    tf = 1.0
    dt = 0.1
    # test once with column space an once with point space because there is no interpolation
    # with point spaces
    for space in [column_space, point_space]
        ...
        short_name =
            ScheduledDiagnostics.output_short_name(diagnostic_every_step)
        ...
        short_name = ScheduledDiagnostics.output_short_name(
